...
case $action in
    ...
    ;;
    apply)
    terraform -chdir="${src_dir}" "${action}" -auto-approve --var-file="../tf-vars/${env}/app-infra-params.tfvars" ${3} ${4}
    ;;
    plan|destroy)
    terraform -chdir="${src_dir}" "${action}" --var-file="../tf-vars/${env}/app-infra-params.tfvars" ${3}
    ;;
    force-unlock)
    ...
    ;;
    import)
    terraform -chdir="${src_dir}" "${action}" --var-file="../tf-vars/${env}/param-component.tfvars" --var-file="../tf-vars/${env}/app-infra-params.tfvars" -generate-config-out="./tf-import-configs/generated.tf"
    ;;
    *)
